...
type ItemStatus = Database['public']['Tables']['items']['Insert']['status'];
...
export const createItem = async (input: CreateItemInput) => {
  ...
  if (!Number.isFinite(priceValue)) throw new Error('価格は数値で入力してください');
  if (priceValue < 100) throw new Error('最低価格は100円です');

  const imageUrl =
    input.image_url?.trim() || input.images?.[0]?.trim() || null;
