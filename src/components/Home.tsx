...
export function Home({ onNavigate }: HomeProps) {
  ...
  // category 概念は UI のみ保持（ロジックでは使わない）
  const [selectedCategory, setSelectedCategory] = useState<"すべて">("すべて");
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {categories.map((category) => (
              <button
                ...
                className="border border-border p-6 bg-card hover:bg-muted hover:border-primary flex flex-col items-center gap-2 rounded-lg transition-all border-primary"
              >
                <div className="w-16 h-16 border-2 border-primary rounded-full flex items-center justify-center text-sm bg-info/20">
                  ALL
                </div>
                <span className="text-foreground">すべて</span>
              </button>
            ))}
          </div>
        </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {visibleItems.map((item) => (
                ...
                >
                  ...
                  <div className="p-4">
                    <p className="text-xs mb-1 text-secondary">その他</p>
                    <h4 className="mb-2 text-foreground line-clamp-2">
                      {item.title}
                    </h4>
